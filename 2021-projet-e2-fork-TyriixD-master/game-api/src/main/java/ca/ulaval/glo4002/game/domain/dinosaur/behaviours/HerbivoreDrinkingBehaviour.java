...
public class HerbivoreDrinkingBehaviour implements DrinkingBehaviour, Serializable {
    ...
    public boolean drink(String resourceType, Weight weight, LunchBox lunchBox) {
        ...
        isThirsty = false;
        return lunchBox.consumeWaterForHerbivores(requiredWaterAmount);
    }

    @Override
    public ResourcesNeeds calculateRequiredResourcesNeeds(Weight weight) {
        int requiredWaterAmount = (int) calculateRequiredWaterAmount(weight);
        return new ResourcesNeeds(0, 0, 0, requiredWaterAmount);
    }
